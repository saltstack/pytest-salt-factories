...
def test_hook_basic_config_defaults(testdir):
    ...
    p = testdir.makepyfile(
        ...
        def test_basic_config_override(request, salt_factories):
            minion_config = salt_factories.configure_salt_minion(request, 'minion-1')
...
def test_keyword_basic_config_defaults(request, salt_factories):
    minion_config = salt_factories.configure_salt_minion(
...
def test_interface_config_defaults(request, salt_factories):
    ...
    minion_config = salt_factories.configure_salt_minion(
        request, "minion-1", config_defaults={"interface": interface}
    )
...
def test_interface_config_overrides(request, salt_factories):
    interface = "172.17.0.1"
    minion_config = salt_factories.configure_salt_minion(
        request, "minion-1", config_overrides={"interface": interface}
    )
    assert minion_config["interface"] == interface
    assert minion_config["interface"] != "127.0.0.1"
...
def test_provide_user(request, salt_factories, configure_kwargs):
    minion_config = salt_factories.configure_salt_minion(request, "minion-1", **configure_kwargs)
